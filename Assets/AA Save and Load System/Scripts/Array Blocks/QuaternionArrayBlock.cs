...
{
    [System.Serializable]
    public class QuaternionArrayBlock
    {
        public string dataName;
        public string[] value;

        public QuaternionArrayBlock(string dataName, Quaternion[] value, bool encryptData)
        {
            if (encryptData)
            {
                ...
                for (int i = 0; i < value.Length; i++)
                {
                    ...
                    this.value[i] += value[i].z.ToString() + "#";
                    this.value[i] += value[i].w.ToString() + "#";
